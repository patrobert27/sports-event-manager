...
// valors de sortida nets per quan creem la pista o quan el profe tanca el formulari
const defaultValues = {
  name: "",
  location: "",
  number_of_courts: 1,
...
// aquest formulari serveix per donar d'alta una instal·lació o pista de joc nova al centre escolar
export default function AddField({ isOpen, onClose, onCreate }) {
  
  // preparem el formulari per a lligar inputs i comprovar que no estiguin buits al desar
  const {
    register,
    handleSubmit,
    reset,
  ...
  // handler de submit per enviar la nova pista cap al thunk de creacio
  const submit = async (data) => {
    try {
      ...
      // tanquem el panell deplegable automaticament un cop creat
      onClose();
      
    } catch (error) {
      // deixem passar l'error perque Redux global ja el tracta des de la graella de dalt
    }
  };

  // vigilem si es tanca el panell de creació per fer un reset de seguretat
  useEffect(() => {
    if (isOpen) {
      reset(defaultValues);
    }
    ...
    <div className="mt-4 bg-white rounded-3xl p-8 border border-primary/10 shadow-[0_20px_50px_rgba(79,168,245,0.12)] animate-in slide-in-from-top-6 fade-in duration-300">
      ...
      >
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          ...
          {/* Nom del camp: obligatori per a no tindre pistes anònimes que confonguin els alumnes */}
          ...
          {/* Ubicació del camp: opcional per a descriure la localització o adreça */}
          <div className="space-y-1.5">
            <label className="text-sm font-bold text-dark/70 ml-1">
              Ubicació / Adreça
            </label>
        ...
        {/* Informació de pistes esportives de la instal·lació */}
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
          
          {/* Número de pistes: obligatori per a saber com organitzar encreuaments de partits simultanis */}
          <div className="space-y-1.5">
        ...
        {/* Botons d'acció per desar o avortar */}
