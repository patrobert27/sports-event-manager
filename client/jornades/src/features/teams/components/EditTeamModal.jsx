...
import { updateTeam } from "../teamThunks";
import TeacherAutocomplete from "../../../components/ui/TeacherAutocomplete";
import { createPortal } from "react-dom";
import { X, Shield } from "lucide-react";

// aquest modal serveix per canviar el tutor, el nom de l'equip o els colors a nivell de base de dades
export default function EditTeamModal({ team, competitionId, onClose }) {
  const dispatch = useDispatch();
  const [localError, setLocalError] = useState(null);

  // useForm pre-carregat amb els valors actuals del team a modificar
  const {
    register,
    handleSubmit,
    control,
    ...
    defaultValues: {
      name: team?.name || "",
      teacher_id: team?.teacher?.id || null,
  ...
  // efecte per carregar en calent la informació si canviem d'equip per editar
  useEffect(() => {
    if (team) {
      reset({
        name: team.name || "",
  ...
  // handler de submit per desar l'edició asíncronament a Redux
  const onSubmit = async (data) => {
    try {
      ...
      await dispatch(
        updateTeam(team.id, payload, competitionId)
      );
                ...
                render={({ field }) => {
                  return (
                    <TeacherAutocomplete
                      ...
                      initialName={team?.teacher
                        ? `${team.teacher.first_name} ${team.teacher.last_name}`
                        : ""}
                    />
                  );
                }}
